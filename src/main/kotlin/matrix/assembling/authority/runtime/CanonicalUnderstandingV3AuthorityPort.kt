...
internal object UnderstandingV3AuthorityProjection {
    fun project(
        ...
        val subject = if (interpretationResolved) {
            role(claim.subjectReferent, candidates.mapValues { it.value.entityRef })
        } else {
            MipEntityRef(
        ...
        return MipClaimV1(
            ...
            source = role(claim.sourceReferent, candidates.mapValues { it.value.entityRef }),
            subject = subject,
            target = role(claim.targetReferent, candidates.mapValues { it.value.entityRef }),
            owner = role(claim.ownerReferent, candidates.mapValues { it.value.entityRef }),
            perspective = role(claim.perspectiveReferent, candidates.mapValues { it.value.entityRef }),
